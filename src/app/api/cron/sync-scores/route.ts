...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const week of weeksToSync) {
      ...
      const weekResults = {
        week,
        preseason: await syncWeekScores(week, "preseason", currentSeason),
        regular: await syncWeekScores(week, "regular", currentSeason),
      };
    ...
    console.log(
      `Automated score sync completed: ${totalSuccessful}/${totalGames} games updated`
    );

    return NextResponse.json({
      success: true,
      message: `Score sync completed: ${totalSuccessful}/${totalGames} games updated`,
      summary: {
        totalGames,
        totalSuccessful,
